...
//
// (InstanceID, TemplateID) 在 deleted_at IS NULL 的活跃行内唯一；
// 通过 partial unique index 实现 —— Postgres 里 NULL 之间互不相等，
// 不能简单把 DeletedAt 塞进联合索引，必须用 WHERE 子句。
type IntegrationInstallation struct {
	ID              uuid.UUID      `json:"id" gorm:"type:uuid;primaryKey"`
	TemplateID      uuid.UUID      `json:"template_id" gorm:"type:uuid;not null;index;uniqueIndex:uk_install_instance_tpl_active,priority:2,where:deleted_at IS NULL"`
	TemplateVersion string         `json:"template_version" gorm:"type:varchar(50)"`
	InstanceID      uuid.UUID      `json:"instance_id" gorm:"type:uuid;not null;index;uniqueIndex:uk_install_instance_tpl_active,priority:1,where:deleted_at IS NULL"`
	...
	Status          string         `json:"status" gorm:"type:varchar(20);default:pending;index"` // pending/success/failed/uninstalled
